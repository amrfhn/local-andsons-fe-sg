  ...
  <div id="showcase-listing" :class="`showcase-listing ${category.slug}`">
    <div v-if="showcaseProducts.length > 0" class="products-container">
      <div class="showcase-header tw-text-center">
        <div class="tw-font-black md:tw-text-6xl tw-text-4xl" v-html="title"></div>
        <p class="description" v-html="description" />
      </div>
      <div
        :class="
          `products md:tw-grid md:tw-gap-16 ${columnsClass} tw-ml-0 md:tw-ml-auto md:tw-mr-auto tw-mt-10 md:tw-mt-0 tw-w-full tw-flex tw-flex-wrap`
        "
      >
        <ShowcaseProductItem
...
<script>
import ShowcaseProductItem from '@/components/ShowcaseProductItem'
export default {
  ...
  computed: {
    ...
    columnsClass: function() {
      switch (this.showcaseProducts.length) {
        ...
        case 5:
          return 'md:tw-grid-cols-5 tw-w-full'
        default:
          return 'md:tw-grid-cols-4 tw-w-full'
      }
    }
  }
}
