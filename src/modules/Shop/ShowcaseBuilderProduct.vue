  ...
  <div id="showcase-listing" :class="`showcase-listing ${category.slug}`">
    <div v-if="showcaseProducts.length > 0" class="products-container">
      <div class="showcase-header tw-text-center">
        <div class="tw-font-black md:tw-text-6xl tw-text-4xl md:tw-mb-4" v-html="title" />
        <p class="description" v-html="description" />
      </div>
      <div
        class="products md:tw-grid lg:tw-gap-14 md:tw-ml-auto md:tw-mr-auto tw-gap-10 tw-mt-10 md:tw-mt-0 tw-w-full tw-flex tw-flex-wrap tw-place-items-center tw-items-stretch"
        :class="columnsClass"
      >
        <ShowcaseBuilderProductItem
      ...
      <router-link
        ...
        :to="`/product/${this.$route.params.catalogue}/select`"
      >
        GET&nbsp;TREATMENT&nbsp;PLANS
      </router-link>
    </div>
...
<script>
import ShowcaseBuilderProductItem from '@/components/ShowcaseBuilderProductItem.vue'
export default {
  components: { ShowcaseBuilderProductItem },
